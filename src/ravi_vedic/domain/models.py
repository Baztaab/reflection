...
from ravi_vedic.errors import InputValidationError, InvariantViolationError
...
class BirthInput:
    ...
    def __post_init__(self) -> None:
        if self.local_datetime.tzinfo is not None:
            raise InputValidationError("local_datetime must be timezone-naive; timezone_id is canonical")
        if self.calendar != "gregorian":
            raise InputValidationError("MVP v1 supports gregorian calendar only")
        if not -90.0 <= self.latitude_deg <= 90.0:
            raise InputValidationError("latitude_deg must be in [-90, 90]")
        if not -180.0 <= self.longitude_deg < 180.0:
            raise InputValidationError("longitude_deg must be in [-180, 180)")
        if self.time_uncertainty_seconds < 0:
            raise InputValidationError("time_uncertainty_seconds must be non-negative")
        if self.fold not in (None, 0, 1):
            raise InputValidationError("fold must be None, 0, or 1")
