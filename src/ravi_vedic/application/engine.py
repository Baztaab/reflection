...
class RaviEngine:
    ...
    def __post_init__(self) -> None:
        ...
        if snapshot.canon_id != RAVI_VEDIC_MVP_V1.canon_id:
            raise UnsupportedPolicyError(f"unsupported canon_id: {snapshot.canon_id}")
        ...
        for name in ("zodiac_policy_id", "ayanamsha_policy_id", "node_policy_id"):
            ...
            if actual != expected:
                raise UnsupportedPolicyError(f"unsupported astronomy.{name}: {actual}")
        ...
        if snapshot.charts.varga_policy_ids != RAVI_VEDIC_MVP_V1.charts.varga_policy_ids:
            raise UnsupportedPolicyError("this engine requires the pinned RAVI chart policy set")
        if snapshot.policy_manifest_sha256 != RAVI_VEDIC_MVP_V1.policy_manifest_sha256:
            raise UnsupportedPolicyError("unsupported policy manifest")
